...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!status && action) {
      ...
      else if (["ofd","out_for_delivery","out for delivery"].includes(action)) { status = "OUT_FOR_DELIVERY"; eta = addMinutes(new Date(), 60); }
      else return json(res, 422, { code:"VALIDATION_ERROR", issues:[{ path:"action", message:"unknown action" }] });
    }
    if (!status) status = next(prevStatus);
    if (!ORDER_STATUSES.includes(status)) return json(res, 422, { code: "VALIDATION_ERROR", issues:[{ path:"status", message:"invalid status" }]});
    ...
    return json(res, 200, { ok:true, order: hydrateRow(upd), debug:{ decidedBy: action ? "action" : (statusParam ? "explicit-status" : "auto-advance"), action: action || null, statusParam: statusParam || null } });
  } catch (e) {
